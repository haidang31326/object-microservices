...
public class InventoryController {
    ...
    public  @ResponseBody ResponseEntity<VenueInventoryResponse> inventoryGetVenueById(@Positive @PathVariable("venueId") Long venueId) {
    ...
    public ResponseEntity<EventInventoryResponse> inventoryGetEventById(@Positive @PathVariable("eventId") Long eventId) {

        return ResponseEntity.ok(inventoryService.getEventInventory(eventId));
    }
    @PutMapping("/inventory/event/{eventId}/capacity/{capacity}")
    public ResponseEntity<Void> updateEventCapacity(@Positive @PathVariable("eventId") Long eventId, @Positive @PathVariable("capacity") Long capacity) {
        inventoryService.updateEventCapacity(eventId, capacity);
        return ResponseEntity.ok().build();
    }
    @PostMapping("/inventory/venue/create")
    public ResponseEntity<VenueInventoryResponse> createVenue(@Valid @RequestBody Venue venue) {
        return ResponseEntity.ok(inventoryService.createVenue(venue));
    }
    @PostMapping("/inventory/event/create/venue/{venueId}")
    public ResponseEntity<EventInventoryResponse> createEvent(@Valid @RequestBody Event event,@Positive @PathVariable Long venueId) {
        return ResponseEntity.ok(inventoryService.createEvent(event, venueId));
    }

    @PutMapping("/inventory/event/{eventId}")
    public ResponseEntity<EventInventoryResponse> updateEvent(@Positive @PathVariable("eventId") Long eventId, @RequestBody Event updatedEvent) {
        return ResponseEntity.ok(inventoryService.updateEvent(eventId, updatedEvent));
    }

    @DeleteMapping("/inventory/event/{eventId}")
    public ResponseEntity<Void> deleteEvent(@Positive @PathVariable("eventId") Long eventId) {
        inventoryService.deleteEvent(eventId);
        return ResponseEntity.ok().build();
    }

    @PutMapping("/inventory/event/{eventId}/restore/{capacity}")
    public ResponseEntity<Void> restoreEventCapacity(@Positive @PathVariable("eventId") Long eventId, @Positive @PathVariable("capacity") Long capacity) {
